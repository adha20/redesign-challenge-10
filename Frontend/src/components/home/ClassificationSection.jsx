...
export default function ClassificationSection() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      if (scrollRef.current) {
        scrollRef.current.scrollBy({ left: 1069, behavior: 'smooth' });
      }
    }, 4000);
  ...
  const handleScrollEvent = () => {
    if (scrollRef.current) {
      ...
      const newIndex = Math.round(scrollLeft / 1069);
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        if (newIndex >= 18) {
          ...
          scrollRef.current.scrollLeft = (newIndex - 9) * 1069;
        } 
  ...
  useEffect(() => {
    if (scrollRef.current && !isInitialized.current) {
      scrollRef.current.scrollLeft = 9 * 1069;
      isInitialized.current = true;
    }
  }, []);

  return (
    <section className="w-full bg-white pt-[50px] pb-[100px] flex flex-col items-center relative overflow-hidden">
      
      {/* Title & Subtitle */}
      <div className="flex flex-col gap-[10px] items-center text-center px-4 max-w-[1015px] mx-auto w-full">
        <h2 className="text-[47px] font-extrabold leading-[1.2] bg-clip-text text-transparent bg-gradient-to-b from-[#ce2323] to-[#681212] pb-[10px] -mb-[10px]">
      ...
      <div className="relative mt-[60px] w-full">
        ...
        <button 
          onClick={scrollLeft}
          className="absolute left-[calc(50%-550.5px)] top-[123px] -translate-y-1/2 z-10 text-[#981b1b] cursor-pointer bg-transparent border-none outline-none hover:scale-110 transition-transform"
          aria-label="Previous"
        >
        ...
        <button 
          onClick={scrollRight}
          className="absolute right-[calc(50%-550.5px)] top-[123px] -translate-y-1/2 z-10 text-[#981b1b] cursor-pointer bg-transparent border-none outline-none hover:scale-110 transition-transform"
          aria-label="Next"
        >
          <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3">
        ...
        <div 
          ...
          className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-[78px] px-[calc(50%-495.5px)] pt-[4px] pb-[10px]"
        >
          {extendedClassifications.map((item) => (
            <div 
              key={item.uniqueId} 
              className="flex-none snap-center w-[991px] h-[238px] rounded-[16px] bg-white border border-gray-100 shadow-[inset_0px_4px_6px_rgba(0,0,0,0.15),0px_4px_4px_rgba(0,0,0,0.05)] flex gap-[14px] items-start px-[36px] py-[36px]"
            >
              {/* Icon */}
              <div className="relative shrink-0 w-[166px] h-[166px]">
                <img src={item.img} alt={item.title} className="w-full h-full object-contain" />
              </div>

              {/* Text */}
              <div className="flex flex-col gap-[8px] flex-1">
                <h3 className="text-[36px] text-center w-full font-extrabold leading-[1.2] bg-clip-text text-transparent bg-gradient-to-b from-[#ce2323] to-[#681212] pb-[10px] -mb-[10px]">
                  {item.title}
                </h3>
                <p className="text-[21px] font-normal text-light-black leading-[1.5] text-justify max-w-[779px] mx-auto">
                  {item.desc}
                </p>
              </div>
            </div>
        ...
        <div className="flex justify-center gap-2 mt-[20px]">
          {classifications.map((_, index) => (
            <button 
              ...
              className={`w-[12px] h-[12px] rounded-full transition-colors ${activeIndex === index ? 'bg-[#981b1b]' : 'bg-gray-100'}`}
      ...
      {/* Description Text */}
      <p className="mt-[50px] text-[21px] font-normal text-light-black leading-[1.5] text-center max-w-[1080px] px-4">
