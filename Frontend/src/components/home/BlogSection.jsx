...
export default function BlogSection() {
  ...
  return (
    <section className="w-full bg-white pt-[50px] pb-[100px] flex flex-col items-center relative z-0">
      ...
      {/* Blog List Wrapper */}
      <div className="relative mt-[60px] flex flex-col items-center justify-center gap-[50px] w-[1080px] z-10">
        
        {/* List Blog */}
        {blogs.map((blog) => (
          <div key={blog.id} className="flex gap-[40px] items-center justify-center w-full">
            {/* Image Container dengan efek frame border biru tebal & shadow besar dari Figma */}
            <div className="w-[260px] h-[146px] rounded-[16px] border-[5px] border-[#2367ce] overflow-hidden shrink-0 shadow-[0px_30px_75px_0px_rgba(0,0,0,0.5)] relative bg-gradient-to-b from-[#2367ce] to-[#b5d3ff]">
            ...
            {/* Teks Blog */}
            <div className="flex flex-col gap-[10px] w-[603px]">
              <h3 className="text-[21px] font-bold text-[#1a1a1a] leading-[1.2]">
                {blog.title}
              </h3>
              <p className="text-[21px] font-normal text-[#1a1a1a] leading-[1.5] text-justify">
                {blog.desc}
              </p>
            </div>
          </div>
        ...
        {/* Tombol Button */}
        <Button variant="primary" className="py-[16px] text-[21px] font-bold mt-[10px]">
          Lihat Selengkapnya
        </Button>
      </div>
    </section>
